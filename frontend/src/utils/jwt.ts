...
export function getUserFromToken(token: string): { id: string; email: string; role: UserRole; name?: string; isTwoFactorEnabled?: boolean; isEmailVerified?: boolean } | null {
    try {
        const decoded = jwtDecode<JwtPayload>(token);
